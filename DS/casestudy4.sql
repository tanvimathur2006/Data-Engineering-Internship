...
SELECT count(Distinct node_id) as countofnodes from customer_nodes;

-- 2. What is the number of nodes per region?
Select region_id,count(Distinct node_id) as nodesperregion from customer_nodes group by region_id;

-- 3. How many customers are allocated to each region?
SELECT region_id, count(Distinct customer_id) as customercount from customer_nodes group by region_id;

-- 4. How many days on average are customers reallocated to a different node?
SELECT ROUND(AVG(DATEDIFF(DAY, start_date, end_date)), 2) AS Avg_days FROM customer_nodes WHERE end_date != '9999-12-31';  

-- 5. What is the median, 80th and 95th percentile for this same reallocation days metric for each region?
SELECT DISTINCT r.region_name,
    PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY DATEDIFF(DAY, start_date, end_date)) OVER (PARTITION BY r.region_name) AS Median,
    PERCENTILE_CONT(0.80) WITHIN GROUP (ORDER BY DATEDIFF(DAY, start_date, end_date)) OVER (PARTITION BY r.region_name) AS P80,
    PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY DATEDIFF(DAY, start_date, end_date))OVER (PARTITION BY r.region_name) AS P95 
    FROM customer_nodes cn JOIN regions r ON cn.region_id = r.region_id
    WHERE cn.end_date != '9999-12-31';
    
-- 6. What is the unique count and total amount for each transaction type?
SELECT txn_type, COUNT(*) AS transaction_count, SUM(txn_amount) AS total_amount FROM Customer_Transactions GROUP BY txn_type;
...
SELECT 
    ...
    SUM(total_deposit) AS Total_Deposit_Amount
FROM ( SELECT customer_id, COUNT(*) AS deposit_count,SUM(txn_amount) AS total_deposit 
    FROM Customer_Transactions WHERE txn_type = 'deposit' GROUP BY customer_id) AS deposits;
...
(SELECT customer_id,YEAR(txn_date) AS yr,MONTH(txn_date) AS mn,
SUM(CASE WHEN txn_type='deposit'THEN 1 ELSE 0 END) AS deposit_count,
SUM(CASE
   WHEN txn_type IN ('purchase','withdrawal')THEN 1 ELSE 0 END) AS other_count
    FROM Customer_Transactions
    GROUP BY customer_id,YEAR(txn_date), MONTH(txn_date))
SELECT yr,mn,COUNT(*) AS customer_count FROM monthly_activity WHERE deposit_count > 1 AND other_count >= 1 GROUP BY yr,mn;